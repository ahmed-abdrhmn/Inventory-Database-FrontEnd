...
abstract class PackageService {
    abstract getList(): Promise<Package[]>;
...
class ServerPackageService extends PackageService{
    async getList() : Promise<Package[]> {
        let resp = await fetch('http://localhost:5230/api/package');
    ...
    async deleteItem(id: number): Promise<void>{
        let resp = await fetch('http://localhost:5230/api/package/' + id, {method: 'DELETE'});
    ...
    async updateItem(id: number, item: any): Promise<void> {
        let resp = await fetch('http://localhost:5230/api/package/' + id, {
            method: 'PUT', 
            body: JSON.stringify(item),
            headers: new Headers({'Content-Type':'application/json'})
        });
...
export {PackageService, ServerPackageService};